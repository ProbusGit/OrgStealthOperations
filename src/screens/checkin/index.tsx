...
import {
  useStartDayMutation,
  useEndDayMutation,
  useGetStartEndDayDetailsQuery,
} from '../../redux/services/activity/ActivitySlice';
        ...
        await RNLocation.requestSettings({
          android: {
            rationale: {
              ...
              message:
                'This app needs location services to track your work hours.',
  ...
  const toggleCheckInOut = async (mode = '') => {
    ...
    if (providers === false) {
      ...
      ReactNativeForegroundService.update({
        ...
        message:
          'GPS is disabled. Tracking has stopped. Please check if your location is turned on.',
        icon: 'ic_launcher',
        visibility: 'public',
        ServiceType: 'location',
        number: '1',
      ...
      if (!employeeId) {
        Alert.alert(
          'Error',
          'Employee ID is not available. Please try again later.',
        );
        return;
      }
      ...
      if (!deviceInfo.mid || !deviceInfo.mip) {
        Alert.alert(
          'Error',
          'Device information is not available. Please try again later.',
        );
        return;
      }
      try {
        ...
        if (isDayStarted && mode == 'change') {
          try {
            ...
            if (
              err?.data?.message ===
              'Updated connection string not found in session.'
            ) {
